...
public class Terrain {
    ...
    public Terrain(GameObjectCollection gameObjects,
        ...
        this.noiseGenerator = new NoiseGenerator(seed, windowDimensions.y());
        groundRenderable = new RectangleRenderable(ColorSupplier.approximateColor(BASE_GROUND_COLOR));
    }
    public float groundHeightAt(float x) { return this.noiseGenerator.getPosition(x); }
    public void createInRange(int minX, int maxX) {
        int startXCoord = (int) Math.floor(groundHeightAt(minX) / Block.SIZE) * Block.SIZE;
        int endXCoord = (int) Math.ceil(groundHeightAt(maxX) / Block.SIZE) * Block.SIZE;
        for(int xCoord = startXCoord; xCoord <= endXCoord; xCoord+=Block.SIZE){
            int yCoord = (int) Math.floor(groundHeightAt(xCoord) / Block.SIZE) * Block.SIZE;
    ...
    class NoiseGenerator{
        ...
        private float maxY;
        NoiseGenerator(int seed, float maxY){
            this.startingX = new Random().nextInt(seed);
            this.maxY = maxY;
        }
        public float getPosition(float x){
            return (float)(maxY - maxY * (((Math.sin(2 * (x + this.startingX)) + Math.sin(Math.PI * (x + this.startingX)))
                    / 8.0) + 0.25));
        }
